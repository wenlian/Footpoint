...
public class ViewUtils {
    ...
    public static void calculateLegendInformation(List<? extends BaseModel> _Models, float _StartX, float _EndX, Paint _Paint) {
        float textMargin = ViewUtils.dpToPx(3.f);
        float lastX = _StartX;
        ...
        for (BaseModel model : _Models) {
            //Log.i(LOG_TAG,"model:"+model.getLegendLabel());
            if (!model.isIgnore()) {
    ...
    public static void calculateLegendInformationShowEnd(List<? extends BaseModel> _Models, float _StartX, float _EndX, Paint _Paint) {
        float textMargin = ViewUtils.dpToPx(10.f);
        float lastX = _StartX;

        // calculate the legend label positions and check if there is enough space to display the label,
        // if not the label will not be shown
        float lastModelWidth = 0;
        if (_Models.size() > 0){
            lastModelWidth = _Models.get(_Models.size() -1).getLegendBounds().width();
        }
        for (int i = 0 ;i <_Models.size();i++) {
            BaseModel model = _Models.get(i);
            //Log.i(LOG_TAG,"i:"+i+";label:"+model.getLegendLabel()+";bounds:"+model.getLegendBounds());
            //calculate the 1st and last text's position
            if (i == 0 || i == _Models.size() -1) {//
                lastX += model.getLegendBounds().width();
                model.setShowLabel(true);
            } else if (!model.isIgnore()) {
                Rect textBounds = new Rect();
                RectF legendBounds = model.getLegendBounds();
                ...
                // check if the text is too big to fit on the screen
                if (centeredTextPos + textBounds.width() > _EndX - textMargin-lastModelWidth) {
                    model.setShowLabel(false);
                } else {
                    // check if the current legend label overrides the label before
                    // if the label overrides the label before, the current label will not be shown.
    ...
    //Calculates the maximum text height of numbers
    public static float calculateNumberMaxTextHeight(Paint _Paint) {
        Rect height = new Rect();
